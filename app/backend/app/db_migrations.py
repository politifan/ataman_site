...
def ensure_service_payment_mode_schema(engine: Engine) -> None:
    ...
    if "payment_mode" in existing_columns:
        return

    with engine.begin() as connection:
        connection.execute(
            text("ALTER TABLE services ADD COLUMN payment_mode VARCHAR(32) NOT NULL DEFAULT 'group_only'")
        )
